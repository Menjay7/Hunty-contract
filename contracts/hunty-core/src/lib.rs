...
total_score_sum = match total_score_sum.checked_add(p.total_score as u64) {
    Some(val) => val,
    None => {
        // fallback to saturating add instead of error
        total_score_sum.saturating_add(p.total_score as u64)
    }
};
